...
export async function POST(req) {
  try {
    ...
    if (!process.env.JWT_SECRET) {
      return new Response(JSON.stringify({ message: "JWT_SECRET not set in environment" }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (!user) {
      return new Response(JSON.stringify({ message: "User does not exist" }), { status: 404, headers: { "Content-Type": "application/json" } });
    }
    if (user.role !== "admin") {
